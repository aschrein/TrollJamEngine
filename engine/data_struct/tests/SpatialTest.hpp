...
bool QTreeTest()
{
	using namespace Collections;
	{
		...
		ito( 2 )
		{
			...
			{
				Array< size_t > quart;
				qtree.getCollided( { new_pos.x * 50.0f , 50.0f } , quart );
				total_size += quart.getSize();
			}
		}
	...
	}
	{
		...
		ito( 4 )
		{
			...
			{
				Array< size_t > quart;
				qtree.getCollided( { new_pos.x * 50.0f , new_pos.y * 50.0f , 50.0f , 50.0f } , quart );
				total_size += quart.getSize();
			}
		}
		RETURN_ASSERT( total_size == 1000 );
	}
	{
		...
		ito( 8 )
		{
			...
			{
				Array< size_t > quart;
				qtree.getCollided( { new_pos.x * 50.0f , new_pos.y * 50.0f , new_pos.z * 50.0f , 50.0f , 50.0f , 50.0f } , quart );
				total_size += quart.getSize();
			}
		}
